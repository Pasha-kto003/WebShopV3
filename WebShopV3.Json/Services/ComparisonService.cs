...
namespace WebShopV3.Json.Services
{
    ...
    public class ComparisonService : IComparisonService
    {
        ...
        public ComparisonService(JsonDataService jsonData)
        {
            _jsonData = jsonData;
        }
        ...
        public async Task<List<ComputerComparisonDto>> AnalyzeComputers(List<Computer> computers)
        {
            ...
            foreach (var computer in computers)
            {
                ...
                // Получаем компоненты компьютера из JSON
                var components = await GetComputerComponents(computer.Id);
        ...
        private async Task<List<Component>> GetComputerComponents(int computerId)
        {
            ...
            // Загружаем характеристики для каждого компонента
            foreach (var component in components)
            {
                ...
                component.ComponentCharacteristics = characteristics
                    ...
                    .Select(c => new ComponentCharacteristic
                    {
                        ...
                        Value = componentCharacteristics
                            .FirstOrDefault(cc => cc.ComponentId == component.Id &&
                                                 cc.CharacteristicId == c.Id)?
                            .Value ?? ""
                    })
                    .ToList();
            }
        ...
        private async Task<ComponentComparisonDto> AnalyzeComponent(Component component)
        {
            ...
            // Добавляем характеристики из ComponentCharacteristics
            if (component.ComponentCharacteristics != null)
            {
            ...
            // Добавляем основные поля как характеристики
            if (!string.IsNullOrEmpty(component.Socket))
                dto.Characteristics["Socket"] = component.Socket;

            if (!string.IsNullOrEmpty(component.MemoryType))
                dto.Characteristics["Тип памяти"] = component.MemoryType;

            if (!string.IsNullOrEmpty(component.FormFactor))
                dto.Characteristics["Форм-фактор"] = component.FormFactor;
        ...
        private void ParseSpecifications(string? specifications, ComponentComparisonDto dto)
        {
            ...
            {
                // Извлекаем чипсет
                var chipsetMatch = Regex.Match(specifications, @"(B\d{3}|X\d{3}|Z\d{3}|A\d{3})", RegexOptions.IgnoreCase);
                if (chipsetMatch.Success)
                    dto.Chipset = chipsetMatch.Value;
            }
        }
        ...
        private int CalculateComponentScore(ComponentComparisonDto component)
        {
            ...
            switch (component.Type.ToUpper())
            {
                ...
                case "GPU":
                    ...
                    // Бонус за AMD
                    if (component.GPUModel?.Contains("RX") == true)
                        score += 250;
                    break;
                ...
                case "RAM":
                    ...
                    // Бонус за DDR5
                    if (component.Characteristics.TryGetValue("Тип памяти", out var memoryType) &&
                        memoryType.Contains("DDR5"))
                        score += 100;
                    break;
                ...
                case "SSD":
                    ...
                    // Бонус за NVMe
                    if (component.Characteristics.TryGetValue("Тип накопителя", out var storageType) &&
                        storageType.Contains("NVMe"))
                        score += 150;
                    break;
                ...
                case "MB":
                    // Бонус за чипсет
                    if (!string.IsNullOrEmpty(component.Chipset))
                    {
                        if (component.Chipset.StartsWith("X") || component.Chipset.StartsWith("Z"))
                            score += 200;
                        else if (component.Chipset.StartsWith("B"))
                            score += 100;
                    }

                    // Бонус за количество слотов памяти
                    if (component.Characteristics.TryGetValue("Количество слотов памяти", out var slotsStr) &&
                        int.TryParse(slotsStr, out int slots))
                        score += slots * 20;

                    // Бонус за форм-фактор
                    if (component.Characteristics.TryGetValue("Форм-фактор", out var formFactor))
                    {
                        if (formFactor.Contains("ATX") || formFactor.Contains("E-ATX"))
                            score += 50;
                    }
                    break;

                case "PSU":
                    // Бонус за мощность и сертификат
                    if (component.Characteristics.TryGetValue("Мощность", out var powerStr) &&
                        int.TryParse(powerStr, out int power))
                        score += power / 10;

                    if (component.Characteristics.TryGetValue("Сертификат", out var cert))
                    {
                        if (cert.Contains("Titanium")) score += 200;
                        else if (cert.Contains("Platinum")) score += 150;
                        else if (cert.Contains("Gold")) score += 100;
                        else if (cert.Contains("Bronze")) score += 50;
                    }
                    break;
            }
        ...
        private int CalculateTotalScore(ComputerComparisonDto computer)
        {
            ...
            var componentTypes = computer.Components.Select(c => c.Type).Distinct().ToList();
            int requiredTypes = 0;

            if (componentTypes.Contains("CPU")) requiredTypes++;
            if (componentTypes.Contains("GPU")) requiredTypes++;
            if (componentTypes.Contains("RAM")) requiredTypes++;
            if (componentTypes.Contains("SSD") || componentTypes.Contains("HDD")) requiredTypes++;
            if (componentTypes.Contains("MB")) requiredTypes++;
            if (componentTypes.Contains("PSU")) requiredTypes++;

            if (requiredTypes >= 4) // Минимум 4 основных компонента
            {
                totalScore += requiredTypes * 50; // 50 баллов за каждый компонент
            }

            // Бонус за наличие корпуса и охлаждения
            if (componentTypes.Contains("Case"))
                totalScore += 100;

            if (componentTypes.Contains("Cooler"))
                totalScore += 80;
        ...
        private bool IsBetterStringValue(string newValue, string currentBest, string specName)
        {
            ...
            {
                // Для GPU: RTX > GTX > RX > другие
                if (newVal.Contains("rtx") && !current.Contains("rtx")) return true;
                if (newVal.Contains("rx") && !current.Contains("rtx") && !current.Contains("rx")) return true;
                if (newVal.Contains("gtx") && !current.Contains("rtx") && !current.Contains("rx") && !current.Contains("gtx")) return true;
            ...
            if (specName.Contains("CPU") || specName.Contains("Процессор"))
            {
                ...
                if (newVal.Contains("i3") && !current.Contains("i9") && !current.Contains("i7") && !current.Contains("i5") && !current.Contains("i3")) return true;
                ...
                if (newVal.Contains("ryzen 3") && !current.Contains("ryzen 9") && !current.Contains("ryzen 7") && !current.Contains("ryzen 5") && !current.Contains("ryzen 3")) return true;

                // Более высокие поколения лучше
                var newGen = ExtractGeneration(newVal);
                var currentGen = ExtractGeneration(current);
                return newGen > currentGen;
            }

            if (specName.Contains("RAM") || specName.Contains("Память"))
            {
                // DDR5 > DDR4 > DDR3
                if (newVal.Contains("ddr5") && !current.Contains("ddr5")) return true;
                if (newVal.Contains("ddr4") && !current.Contains("ddr5") && !current.Contains("ddr4")) return true;
                if (newVal.Contains("ddr3") && !current.Contains("ddr5") && !current.Contains("ddr4") && !current.Contains("ddr3")) return true;
            }
        ...
        private int ExtractGeneration(string text)
        {
            var match = Regex.Match(text, @"(\d+)(?:th|nd|rd|st|й|ый|ой)?\s*(?:gen|поколение|поколения)");
            if (match.Success && int.TryParse(match.Groups[1].Value, out int gen))
                return gen;

            // Альтернативный поиск
            match = Regex.Match(text, @"(\d{4})");
            if (match.Success)
            {
